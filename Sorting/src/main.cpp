...
int main() {
    const int n = 10000; // número máximo de elementos en el arreglo
    const int step = 5; // cantidad de elementos a incrementar en cada iteración
    ...
    // arreglos para almacenar los tiempos de ejecución normalizados de cada método
    double y_ins_best1[n/step];
    double y_ins_avg1[n/step];
    double y_ins_worst1[n/step];
    double y_sel_best1[n/step];
    double y_sel_avg1[n/step];
    double y_sel_worst1[n/step];
    double y_mer_best1[n/step];
    double y_mer_avg1[n/step];
    double y_mer_worst1[n/step];
    ...
    for (int i = 0; i < n/step; i++) {
        ...
        // almacenar los tiempos normalizados en los arreglos correspondientes
        y_ins_best1[i] = timess[0];
        y_ins_avg1[i] = timess[1];
        y_ins_worst1[i] = timess[2];
        y_sel_best1[i] = timess[3];
        y_sel_avg1[i] = timess[4];
        y_sel_worst1[i] = timess[5];
        y_mer_best1[i] = timess[6];
        y_mer_avg1[i] = timess[7];
        y_mer_worst1[i] = timess[8];

        // liberar la memoria asignada a timess
        delete[] timess;
    }

    double y_ins_bestt[n/step];
    double y_ins_avgt[n/step];
    double y_ins_worstt[n/step];
    double y_sel_bestt[n/step];
    double y_sel_avgt[n/step];
    double y_sel_worstt[n/step];
    double y_mer_bestt[n/step];
    double y_mer_avgt[n/step];
    double y_mer_worstt[n/step];

    for (int i = 0; i < n/step; i++) {
        y_ins_bestt[i] = (y_ins_best[i] + y_ins_best1[i]) / 2;
        y_ins_avgt[i] = (y_ins_avg[i] + y_ins_avg1[i]) / 2;
        y_ins_worstt[i] = (y_ins_worst[i] + y_ins_worst1[i]) / 2;
        y_sel_bestt[i] = (y_sel_best[i] + y_sel_best1[i]) / 2;
        y_sel_avgt[i] = (y_sel_avg[i] + y_sel_avg1[i]) / 2;
        y_sel_worstt[i] = (y_sel_worst[i] + y_sel_worst1[i]) / 2;
        y_mer_bestt[i] = (y_mer_best[i] + y_mer_best1[i]) / 2;
        y_mer_avgt[i] = (y_mer_avg[i] + y_mer_avg1[i]) / 2;
        y_mer_worstt[i] = (y_mer_worst[i] + y_mer_worst1[i]) / 2;
    }
    ...
    for (int i = 0; i < n/step; i++) {
        cout << i << " " << (y_ins_bestt[i])*1000 << " " << y_ins_avgt[i] << " " << y_ins_worstt[i] << " "
            << y_sel_bestt[i] << " " << y_sel_avgt[i] << " " << y_sel_worstt[i] << " "
            << y_mer_bestt[i] << " " << y_mer_avgt[i] << " " << y_mer_worstt[i] << endl;
    }
    return 0;
}